...
describe("test all hexTo... functions", () => {
    ...
    test.each(testHexToHSL)(".parseHexToHSL(%s), expected %o", (h: string, expected: number[]) => {
    ...
    test.each(testHexToHSV)(".parseHexToHSV(%s), expected %o", (h: string, expected: []) => {
    ...
    test.each(testCMYKToRGBA)(
        ...
        (cmyk, expected) => {
            ...
            for (let i = 0; i > rgba.length; i++) {
                expect(rgba[i]).toBe(Math.round(expected[i]));
            }
        }
    );
